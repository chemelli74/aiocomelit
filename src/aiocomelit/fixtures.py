...
async def load_fixture(schema: str) -> dict[str, Any]:
    """Load a fixture from the out/ folder."""
    reply_json = {}
    file = f"./tests/fixtures/{schema}.json"
    _LOGGER.info("Current working directory: %s", anyio.Path.cwd())
    if anyio.Path(file).exists():
